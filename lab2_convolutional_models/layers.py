...
class Convolution(Layer):
    ...
    def __init__(self, input_layer, num_filters, kernel_size, name, padding='SAME',
                 weights_initializer_fn=variance_scaling_initializer, bias_initializer_fn=zero_init):
...
class FC(Layer):
    ...
    """
        self.input = inputs
        self.out = np.matmul(inputs, np.transpose(self.weights)) + self.bias
        return self.out.copy()
    ...
    """
        self.back = np.dot(grads, self.weights)
    ...
    """
        grad_w = np.matmul(np.transpose(grads), self.input)
        grad_b = np.sum(grads, axis=0)

        w_pairs = [self.weights, grad_w]
        b_pairs = [self.bias, grad_b]

        return w_pairs, b_pairs
...
class ReLU(Layer):
    ...
    """
        self.input = inputs
        self.out = np.maximum(np.zeros(self.input.shape), self.input)
        return self.out.copy()
    ...
    Returns:
      ndarray of shape (N, C, H, W).
    """
        self.back = np.where(self.input >= 0, 1, 0) * grads
        return self.back.copy()
...
class SoftmaxCrossEntropyWithLogits:
    ...
    """
        N, num_classes = x.shape[0], x.shape[1]

        # stable softmax
        stable_x = x - np.amax(x, axis=1, keepdims=True)
        expscores = np.exp(stable_x)  # N x C
        sumexp = np.sum(expscores, axis=1).reshape(N, 1)  # N x 1
        logprobs = np.log(expscores / sumexp)  # N x C
        loss_per_C = (1 / N) * np.sum(y * logprobs, axis=1)  # 1 x C
        self.loss = - np.sum(loss_per_C)  # scalar
    ...
    """
        ...
        # stable softmax
        expscores = np.exp(x - np.amax(x, axis=1, keepdims=True))  # N x C
        sumexp = np.sum(expscores, axis=1).reshape(N, 1)  # N x 1
...
class L2Regularizer:
    ...
    """
        squared_weights = self.weights * self.weights
        self.out = 0.5 * np.sum(squared_weights) * self.weight_decay
    ...
    """
        self.grad_w = self.weight_decay * self.weights
        self.back = [[self.weights.copy(), self.grad_w.copy()]]
